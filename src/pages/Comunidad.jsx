...
const Comunidad = () => {
    ...
    const [searchSuccesful, setSearchSuccessful] = useState(false);
    ...
    return (
        <div className='body_comunidad'>
            <div className='contenido_comunidad'>
                ...
                <h1 className='titulo_findUser'>Más usuarios</h1>
                <div className='usuarios-famosos'>
                    {Array.isArray(users) && users.map(user => (
                        <div key={user.id} className='usuario' onClick={() => navigate(`/perfil/${user.id}`)} style={{ cursor: 'pointer' }}>
                            ...
                            <p>{user.username}{user.isVerified ? <FontAwesomeIcon icon={faCheckCircle} style={{ color: '#F6AE2D', marginLeft: '5px' }} /> : null}</p>
                        </div>
                    ))}
                </div>
                <hr className='decorator-separator-2-lista decorator-separator-red-lista' />
                ...
                <div className='div_reviews'>
                    <h1 className='titulo_seccion_reviews'>Reviews más positivas</h1>
